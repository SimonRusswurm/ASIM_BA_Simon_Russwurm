...
function createMovingObj(element, aPath){
    let clone = element.cloneNode(true);
    clone.id = "clonedElement";
    clone.style.background = "yellow";
    clone.style.height = String(100/32*2) + "%";
    clone.style.width = String(100/46*2) + "%";
    clone.style.borderRadius = "0.5vw";
    clone.style.position = "absolute";
    clone.style.top = String(100/32*aPath[0].y) + "%";
    clone.style.left = String(100/46*aPath[0].x) + "%";

    document.querySelector(".globalgrid").appendChild(clone);
...
let path = pathFromTo(convertlabelToPoint("A"), convertlabelToPoint("SW"));
console.log(path);
let mov = createMovingObj(A, path);
...
function updateMovObjPosition(aMovObj){
    ...
    if(aMovObj.path.length > 1){
        ...
        aMovObj.aDiv.style.top = String((aMovObj.path[0].y-1)*100/32) + "%" ;
        aMovObj.aDiv.style.left = String((aMovObj.path[0].x-1)*100/46) + "%";
    } 
}
